<template>
    ...
    <header class="w-full bg-white/95 shadow-md py-4 px-8 flex items-center justify-between fixed top-0 left-0 z-10 border-b border-gray-200 backdrop-blur-sm">
      <div class="flex items-center gap-3">
        <button @click="voltarParaInicio" class="flex items-center gap-2 hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-orange-700 rounded-lg transition-all duration-200 text-left">
          <span class="inline-flex items-center justify-center w-10 h-10 bg-gradient-to-br from-orange-700 to-orange-800 rounded-lg text-white font-black text-sm shadow-lg hover:shadow-xl transition-shadow duration-200">D</span>
          <div>
      ...
      <button @click="voltarParaInicio" class="text-gray-600 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-orange-700 rounded-lg text-sm font-semibold transition-all duration-200 flex items-center gap-1.5 px-3 py-2 hover:bg-gray-100">
      ...
      <div class="w-full max-w-2xl">
        <div class="bg-white rounded-2xl shadow-2xl p-8 border border-gray-200">
            ...
            <div class="mb-8">
              ...
              <p class="text-gray-600 text-base">Selecione seus arquivos e configure conforme necessário</p>
            </div>
            ...
            <section class="mb-8">
              ...
              <div v-if="selectedFiles.length" class="mt-4">
                <div class="flex justify-between items-center mb-3">
                  ...
                  <button @click="clearFiles" class="text-xs text-gray-500 hover:text-red-600 focus:outline-none focus:ring-1 focus:ring-red-500 rounded px-2 py-1 transition-colors duration-200 font-semibold">Limpar tudo</button>
                </div>
                <div class="max-h-28 overflow-y-auto space-y-1 bg-gradient-to-b from-orange-50 to-gray-50 p-3 rounded-lg border border-gray-200">
                  <div v-for="(file, idx) in selectedFiles" :key="idx" class="flex justify-between items-center text-sm text-gray-700 py-2 px-3 bg-white rounded border border-gray-100">
                    ...
                    <button @click="removeFile(idx)" class="text-gray-400 hover:text-red-600 focus:outline-none focus:ring-1 focus:ring-red-500 ml-2 flex-shrink-0 font-bold rounded transition-colors duration-200">✕</button>
                  </div>
                </div>
              </div>
            </section>
            ...
            <transition name="fade-slide">
              <div v-if="selectedFiles.length > 0" class="space-y-6 py-6 border-t border-gray-200 mt-6">
                ...
                <h3 class="text-sm font-bold text-gray-900 mb-4">Opções avançadas</h3>
                <ConfigPanel 
                ...
                <button 
                  ...
                  :disabled="loading"
                  class="w-full bg-gradient-to-r from-orange-700 to-orange-800 text-white py-4 rounded-lg hover:from-orange-800 hover:to-orange-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-700 disabled:from-gray-400 disabled:to-gray-400 disabled:cursor-not-allowed font-bold transition-all duration-200 flex items-center justify-center gap-2 text-base shadow-lg hover:shadow-xl">
